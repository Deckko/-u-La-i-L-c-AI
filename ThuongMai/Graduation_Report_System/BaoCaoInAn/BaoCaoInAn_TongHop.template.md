  ...
  <div style="margin-left: 20%; font-size: 13pt; line-height: 1.8; margin-bottom: 100px;">
    <table style="border: none; border-collapse: collapse;">
      <tr style="border: none;">
        ...
        <td style="padding: 2px 10px; border: none; font-family: 'Times New Roman', Times, serif;">{{advisor}}</td>
      </tr>
      <tr style="border: none;">
        ...
        <td style="padding: 2px 10px; border: none; font-family: 'Times New Roman', Times, serif;">{{student_1_name}}</td>
      </tr>
      <tr style="border: none;">
        <td style="padding: 2px 10px 2px 0px; font-weight: bold; border: none; font-family: 'Times New Roman', Times, serif;">Lớp:</td>
        <td style="padding: 2px 10px; border: none; font-family: 'Times New Roman', Times, serif;">{{student_1_class}}</td>
      </tr>
      <tr style="border: none;">
        <td style="padding: 2px 10px 2px 0px; font-weight: bold; border: none; font-family: 'Times New Roman', Times, serif;">MSSV:</td>
        <td style="padding: 2px 10px; border: none; font-family: 'Times New Roman', Times, serif;">{{student_1_mssv}}</td>
      </tr>
      <tr style="border: none;">
        ...
        <td style="padding: 2px 10px; border: none; font-family: 'Times New Roman', Times, serif;">{{company}}</td>
      </tr>
    </table>
  </div>
...
<p align="right">
  Sinh viên thực hiện<br/><br/><br/><br/>
  {{student_1_name}}
</p>
...
* **Họ và tên giảng viên hướng dẫn**: {{advisor}}
* **Họ và tên HSSV thực tập**: {{student_1_name}}
* **MSSV**: {{student_1_mssv}}
* **Lớp**: {{student_1_class}}
...
Qua khảo sát thực tế tại **{{company}}**, em nhận thấy hai bài toán lớn chưa có lời giải tối ưu trên các website bán lẻ hiện hành:
...
1. **Truy xuất (Retrieval)**: Khi khách hàng gửi câu hỏi lên chatbot, hệ thống sẽ chuyển câu hỏi đó thành một vector nhúng. Vector Database (Qdrant) sẽ tìm kiếm trong cơ sở dữ liệu các vector mô tả sản phẩm có độ tương đồng ngữ nghĩa cao nhất với câu hỏi của khách hàng.
2. **Tăng cường và Tạo lập (Augmented Generation)**: Hệ thống sẽ lấy thông tin chi tiết của các sản phẩm tìm được ở bước 1 làm ngữ cảnh đầu vào (Context), gộp chung với câu hỏi gốc của khách hàng thành một Prompt hoàn chỉnh gửi lên LLM. Mô hình LLM lúc này chỉ đóng vai trò là một bộ biên dịch ngôn ngữ tự nhiên, đọc hiểu ngữ cảnh được cung cấp để tạo ra câu trả lời chuẩn xác và trung thực 100% dựa trên dữ liệu sản phẩm có thật, hoàn toàn loại bỏ hiện tượng bịa đặt thông tin.
...
Để xác định xem hai đoạn văn bản (ví dụ: câu hỏi của khách hàng và mô tả tính năng của sản phẩm) có tương đồng ngữ nghĩa với nhau hay không, toán học sử dụng phép đo độ tương đồng Cosine (Cosine Similarity). Phép đo này tính góc Cosine giữa hai vector $\mathbf{A}$ và $\mathbf{B}$ trong không gian đa chiều. 
...
#### 3.1.1 Thống kê cấu trúc tài nguyên mã nguồn thực tế (Phân tích Tự động)
Hệ thống được quét và biên dịch tự động trực tiếp từ mã nguồn thực tế của dự án E-commerce DECKKO để đảm bảo tính nhất quán tuyệt đối của tài liệu:
* **Các trang tuyến đường (Routes)**: {{CODE_METADATA_PAGES}}
* **Các Hooks xử lý logic nghiệp vụ (Custom Hooks)**: {{CODE_METADATA_HOOKS}}
* **Thành phần giao diện nguyên tử (Atomic Components)**:
  * Atoms (Thành phần cơ bản): `{{CODE_METADATA_ATOMS}}` tệp tin.
  * Molecules (Thành phần ghép): `{{CODE_METADATA_MOLECULES}}` tệp tin.
  * Organisms (Thành phần phức hợp): `{{CODE_METADATA_ORGANISMS}}` tệp tin.
* **Tổng số lượng sản phẩm được số hóa**: `{{CODE_METADATA_PRODUCTS_COUNT}}` sản phẩm mẫu trong cơ sở dữ liệu giả lập.
...
Sau 8 tuần thực tập tốt nghiệp đầy nỗ lực tại **{{company}}** dưới sự giảng dạy hướng dẫn sát sao của cô **Hồ Thị Thanh Diệu**, em đã hoàn thành toàn bộ các yêu cầu chức năng và kỹ thuật của hệ thống thương mại điện tử DECKKO Classic:
